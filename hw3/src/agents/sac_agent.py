...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action_distribution = self.actor(obs)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        samples = action_distribution.rsample((1,))  # shape (1, batch_size, action_dim)
        log_prob = action_distribution.log_prob(samples)  # shape (1, batch_size)
        entropy = -log_prob.mean(dim=0)  # shape (batch_size,)
        return entropy
        # ENDTODO
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        loss = -(q_values.mean(dim=0) - self.get_temperature() * log_prob).mean()
        # ENDTODO
