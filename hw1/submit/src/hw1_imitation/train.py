...
def run_training(config: TrainConfig) -> list[float]:
    ...
    for epoch in range(num_epochs):
        for batch in loader:
            ...
            if train_steps % config.log_interval == 0:
                wandb.log({'step': train_steps, 'loss': loss.item()})
                logger.log({'train/loss': loss.item()}, step=train_steps)
                 
            if train_steps % config.eval_interval == 0:
                evaluate_policy(
